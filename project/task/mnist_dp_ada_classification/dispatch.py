...
from project.task.mnist_dp_ada_classification.dataset import get_dataloader_generators
from project.task.mnist_dp_ada_classification.models import (
    get_logistic_regression,
    get_net,
)
from project.task.mnist_dp_ada_classification.train_test import (
    get_fed_eval_fn,
    test,
    train,
)
...
def dispatch_train(
    ...
    **kwargs: Any,
    ...
    if train_structure is not None and train_structure.upper() == "MNIST_DP_ADA":
...
def dispatch_data(cfg: DictConfig, **kwargs: Any) -> DataStructure | None:
    ...
    # for MNIST_DP_ADA and the dispatch should return None
    ...
    if client_model_and_data is not None and partition_dir is not None:
        ...
        if client_model_and_data.upper() == "MNIST_DP_ADA_CNN":
            return (
                get_net,
                client_dataloader_gen,
                fed_dataloader_gen,
        ...
        elif client_model_and_data.upper() == "MNIST_DP_ADA_LR":
            return (
                get_logistic_regression,
                client_dataloader_gen,
                fed_dataloader_gen,
